...
def conectar_a_db():
    conn = sqlite3.connect('datos_actividades.db')  # Ajusta la ruta según sea necesario
    return conn

def analizar_reincidencias_y_fallas_tempranas():
    conn = conectar_a_db()
    
    # Realizar la consulta para cargar los datos
    query = "SELECT * FROM actividades"  # Ajusta el nombre de la tabla
